...
enum LPNetdiagError: Error {
    case accessingDNSFailed // Problem accessing the DNS
    case ipResolutionFailed
}
public protocol LPTCPPingDelegate: class {
    func tcpPing(_ pinger: LPTCPPing, output line: String)
    func tcpPing(_ pinger: LPTCPPing, didFailWith error: Error)
}

public class LPTCPPing {
    ...
    public struct Result: CustomStringConvertible {
        ...
        public var description: String {
            if code == 0 || code == LPTCPPing.kRequestStoped {
    ...
    private weak var delegate: LPTCPPingDelegate?
    private let completion: ((Result) -> Void)?
    
    deinit {
        #if DEBUG
        print("LPTCPPing release memory.")
        #endif
    }
    
    public init(host: String, port: UInt16 = 80, count: UInt32, delegate: LPTCPPingDelegate?, completion: ((Result) -> Void)?) {
        self.host = host
        self.port = port
        self.count = count
    ...
    private func run() {
        ...
        if addr.sin_addr.s_addr == INADDR_NONE {
            guard let host = gethostbyname(hostaddr), let h_addr = host.pointee.h_addr_list?[0] else {
                delegate?.tcpPing(self, didFailWith: LPNetdiagError.accessingDNSFailed)
        ...
        guard let ipAddr = inet_ntoa(addr.sin_addr), let ip = String(utf8String: ipAddr) else {
            delegate?.tcpPing(self, didFailWith: LPNetdiagError.ipResolutionFailed)
            return
        }
        ...
        if let completion = completion {
            let code = stopped ? LPTCPPing.kRequestStoped : r
            DispatchQueue.main.async {
