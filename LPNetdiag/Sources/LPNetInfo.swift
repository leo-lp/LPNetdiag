...
public class LPNetInfo: CustomStringConvertible {
    
    /// 网络连接状态改变通知
    public static let reachabilityChangedNotification = Notification.Name(rawValue: "com.lp.netdiag.notification.reachabilityChanged")
    private(set) var reachability: LPReachability?
    /// 获取运营商信息
    ...
    public var description: String {
        ...
        return """
        网络类型：\(reachability?.status.description ?? "unknown")
        网络运营商：\(cellulars?.description ?? "unknown")
        用户外网IP：\(externalNetworkIP ?? "unknown")
        """
    }
}
...
extension LPNetInfo {
    ...
    public func startMonitoring(_ host: String?) {
        guard reachability == nil else { return assert(false, "网络监控已经开启") }
        
        if let host = host, let reachability = LPReachability(host: host) {
            return startMonitoring(reachability)
        }
        
        if let reachability = LPReachability() {
            return startMonitoring(reachability)
        }
        return assert(false, "开启网络监控失败")
    }
    
    private func startMonitoring(_ reachability: LPReachability) {
        reachability.monitorCallback = { (state) in
            LPNetInfo.externalNetworkIP = nil // 当网络状态发生改变后将此属性置为nil
            
            #if DEBUG
            print("⚠️⚠️⚠️ ----------------------------------------- ⚠️⚠️⚠️")
            print("网络状态状态：-> \(state.description)")
            print("⚠️⚠️⚠️ ----------------------------------------- ⚠️⚠️⚠️")
            #endif
            DispatchQueue.main.async {
                NotificationCenter.default.post(name: LPNetInfo.reachabilityChangedNotification, object: nil)
            }
        }
        
        if !reachability.startMonitoring() {
            assert(false, "开启网络监控失败")
        }
