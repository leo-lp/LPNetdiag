...
public class LPReachability {
    private let reachability: SCNetworkReachability
    private var queue: DispatchQueue = DispatchQueue(label: "com.lp.netdiag.queue.reachability")
    ...
    public func startMonitoring() -> Bool {
        ...
        let callback: SCNetworkReachabilityCallBack = { (_, flags, info) in
            let observer = info.map { Unmanaged<LPReachability>.fromOpaque($0).takeUnretainedValue() }
...
extension LPReachability.Status: Equatable {}
