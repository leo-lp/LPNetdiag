...
public protocol LPUDPTracerouteDelegae: class {
    func udpTraceroute(_ traceroute: LPUDPTraceroute, output routelog: String)
    func udpTracerouteDidEnd(in traceroute: LPUDPTraceroute)
}
/// TraceRoute网络监控
/// 主要是通过模拟shell命令traceRoute的过程，监控网络站点间的跳转
/// 默认执行20转，每转进行三次发送测速
public class LPUDPTraceroute {
    ...
    private weak var delegate: LPUDPTracerouteDelegae?
    ...
    ///   - port: 执行端口（默认 30001）
    public init(ttl: UInt32, attempts: UInt32, port: UInt16 = 30001, delegate: LPUDPTracerouteDelegae?) {
        self.maxTTL = ttl
        self.udpPort = port
    ...
    public func startTrace(_ host: String) -> Bool {
        ...
        guard let serverDNSs = LPNetdiag.shared.netInfo.dns(host), !serverDNSs.isEmpty else {
        ...
        while ttl <= maxTTL {
            ...
            for `try` in 0..<maxAttempts {
                ...
                readfds.lp_zero() // FD_ZERO
                readfds.lp_set(recv_sock) // FD_SET
                select(recv_sock + 1, &readfds, nil, nil, &tv)
                
                if readfds.lp_isSet(recv_sock) {
                    timeoutTTL = 0
