...
//  LPNetdiag
//
//  Created by pengli on 2019/11/27.
...
// MARK: - LPICMPPingDelegate
/// A delegate protocol for the LPICMPPing class.
public protocol LPICMPPingDelegate: class {
    ...
    func icmpPing(_ pinger: LPICMPPing, didStartWith address: Data)
    ...
    func icmpPing(_ pinger: LPICMPPing, didFailWith error: Error)
    ...
    func icmpPing(_ pinger: LPICMPPing, didSend packet: Data, sequenceNumber: UInt16)
    ...
    ///         `-simplePing:didFailToSendPacket:sequenceNumber:error:` delegate callback (unless you
    ///     stop the object before you get the callback).  These callbacks are currently delivered synchronously from within `-sendPingWithData:`, but this synchronous behaviour is not considered API.
    func icmpPing(_ pinger: LPICMPPing, didFailToSend packet: Data, sequenceNumber: UInt16, error: Error)
    ...
    func icmpPing(_ pinger: LPICMPPing, didReceivePingResponse packet: Data, sequenceNumber: UInt16)
    ...
    func icmpPing(_ pinger: LPICMPPing, didReceiveUnexpected packet: Data)
}
...
public class LPICMPPing {
    ...
    deinit {
        ...
        assert(socket == nil)
        #if DEBUG
        print("LPICMPPing release memory.")
        #endif
    }
    ...
    public func start() {
        ...
        CFHostSetClient(host, lp_hostResolveCallback(_:typeInfo:error:info:), &context)
    ...
    public func sendPing(with data: Data?) {
        ...
        case AF_INET:
            packetOpt = pingPacket(withType: LPICMPv4TypeEcho.request.rawValue, payload: payload, requiresChecksum: true)
        case AF_INET6:
            packetOpt = pingPacket(withType: LPICMPv6TypeEcho.request.rawValue, payload: payload, requiresChecksum: false)
        default:
            assert(false)
        }
    ...
    private func startWithHostAddress() {
        ...
        if err != 0 {
            ...
            socket = CFSocketCreateWithNative(nil, fd, CFSocketCallBackType.readCallBack.rawValue, lp_socketReadCallback, &context)
            assert(socket != nil)
    ...
    private func pingPacket(withType type: UInt8, payload: Data, requiresChecksum: Bool) -> Data {
        var icmpHeader = LPICMPHeader(type: type, code: 0, checksum: 0, identifier: identifier.bigEndian, sequenceNumber: nextSequenceNumber.bigEndian)
        ...
        if requiresChecksum {
            ...
            var checksum = lp_in_cksum(packet)
    ...
    private func validatePing4ResponsePacket(_ packet: inout Data, sequenceNumber: inout UInt16) -> Bool {
        guard let icmpHeaderOffset = LPICMPPing.icmpHeaderOffset(inIPv4Packet: packet) else { return false }
        var icmpData = Data(packet[icmpHeaderOffset..<packet.count])
        return icmpData.withUnsafeBytes { (body) -> Bool in
            if let baseAddress = body.baseAddress {
                let icmpPtr = baseAddress.assumingMemoryBound(to: LPICMPHeader.self)
                ...
                calculatedChecksum = lp_in_cksum(icmpData)
                //icmpPtr.pointee.checksum = receivedChecksum
                if receivedChecksum == calculatedChecksum
                    && ((icmpPtr.pointee.type == LPICMPv4TypeEcho.reply.rawValue)
    ...
    private func validatePing6ResponsePacket(_ packet: inout Data, sequenceNumber: inout UInt16) -> Bool {
        guard packet.count >= MemoryLayout<LPICMPHeader>.size else { return false }
        return packet.withUnsafeBytes { (body) -> Bool in
            if let baseAddress = body.baseAddress {
                ...
                if (icmpPtr.pointee.type == LPICMPv6TypeEcho.reply.rawValue) && (icmpPtr.pointee.code == 0) && icmpPtr.pointee.identifier.bigEndian == identifier {
                    let sequenceNumberValue = icmpPtr.pointee.sequenceNumber.bigEndian
                    if validateSequenceNumber(sequenceNumberValue) {
                        sequenceNumber = sequenceNumberValue
                        return true
    ...
    static private func icmpHeaderOffset(inIPv4Packet packet: Data) -> Int? {
        ...
        guard packet.count >= (MemoryLayout<LPIPv4Header>.size + MemoryLayout<LPICMPHeader>.size) else { return nil }
        return packet.withUnsafeBytes { (body) -> Int? in
            if let baseAddress = body.baseAddress {
                let ipPtr = baseAddress.assumingMemoryBound(to: LPIPv4Header.self)
                if ((ipPtr.pointee.versionAndHeaderLength & 0xF0) == 0x40) && (ipPtr.pointee.protocol == IPPROTO_ICMP) { // IPv4
                    ...
                    if (packet.count >= (ipHeaderLength + MemoryLayout<LPICMPHeader>.size)) {
...
private func lp_in_cksum(_ buffer: Data) -> UInt16 {
...
private func lp_hostResolveCallback(_ theHost: CFHost, typeInfo: CFHostInfoType, error: UnsafePointer<CFStreamError>?, info: UnsafeMutableRawPointer?) -> Void {
    ...
    let obj = unsafeBitCast(info, to: LPICMPPing.self)
...
private func lp_socketReadCallback(_ s: CFSocket?, type: CFSocketCallBackType, address: CFData?, data: UnsafeRawPointer?, info: UnsafeMutableRawPointer?) -> Void {
    guard let info = info else { return }
...
public enum LPICMPPingAddressStyle {
...
private enum LPICMPv4TypeEcho: UInt8 {
...
private enum LPICMPv6TypeEcho: UInt8 {
